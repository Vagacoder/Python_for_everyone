## Turns a number < 1,000,000 into its English name.
# @param number a positive integer < 1,000,000
# @return the name of the number (e.g., “two hundred seventy four”)

## There are 3 sub-functions to turn number <10 ; < 20; and <100, please see below
...
def intName(number):
    ...
    name = removeLeadingSpace(name)
    return name

## Turns a number < 999,999,999 into its English name.
...
## this remove leading space(s)
# @para string, input string
# @return , truncated string

def removeLeadingSpace(string):
    while string[0] == ' ':
        string = string[1:]
    return string